...
'[{"id":"a","text":"Junos OS uses a monolithic kernel with all processes running in kernel space","is_correct":false},{"id":"b","text":"Junos OS has a modular architecture with separate user-space processes for each protocol","is_correct":true},{"id":"c","text":"Junos OS is based on the Linux kernel and runs all routing protocols as kernel modules","is_correct":false},{"id":"d","text":"Junos OS uses a real-time operating system with no user-space processes","is_correct":false}]'::jsonb,
'Junos OS is built on the FreeBSD kernel and uses a modular architecture. Each routing protocol (OSPF, BGP, IS-IS, etc.) runs as a separate user-space daemon.',
...
'[{"id":"a","text":"Configuration mode (configure exclusive)","is_correct":false},{"id":"b","text":"Operational mode","is_correct":true},{"id":"c","text":"Monitor mode","is_correct":false},{"id":"d","text":"Enable mode","is_correct":false}]'::jsonb,
'Junos CLI has two main modes: operational mode (>) for monitoring and troubleshooting, and configuration mode (#) for making changes.',
...
'[{"id":"a","text":"show ospf neighbor","is_correct":true},{"id":"b","text":"show ospf database","is_correct":false},{"id":"c","text":"show ospf interface","is_correct":false},{"id":"d","text":"show ospf adjacency","is_correct":false}]'::jsonb,
'The correct command is "show ospf neighbor". It displays all OSPF neighbors, their state (Full, 2-Way, etc.), interface, and neighbor ID.',
...
'[{"id":"a","text":"MED (Multi-Exit Discriminator)","is_correct":false},{"id":"b","text":"Local Preference","is_correct":false},{"id":"c","text":"AS_PATH","is_correct":true},{"id":"d","text":"Community","is_correct":false}]'::jsonb,
'AS_PATH is used for loop prevention in BGP. When a BGP router receives an update, it checks if its own AS number is in the AS_PATH attribute.',
...
'[{"id":"a","text":"Accept all BGP routes","is_correct":false},{"id":"b","text":"Reject all BGP routes","is_correct":false},{"id":"c","text":"Accept all active BGP routes","is_correct":true},{"id":"d","text":"Only accept default routes","is_correct":false}]'::jsonb,
'By default, Junos accepts all BGP routes from EBGP peers. However, best practice is to apply import and export policies.',
...
'[{"id":"a","text":"accept","is_correct":true},{"id":"b","text":"discard","is_correct":true},{"id":"c","text":"permit","is_correct":false},{"id":"d","text":"forward","is_correct":false}]'::jsonb,
'Junos firewall filter actions include: accept (allow the packet), discard (silently drop), reject (drop with ICMP unreachable), and count (count packets without dropping).',
...
'[{"id":"a","text":"Changes take effect immediately as you type them","is_correct":false},{"id":"b","text":"Changes are staged and applied atomically with the commit command","is_correct":true},{"id":"c","text":"Changes are automatically saved every 5 minutes","is_correct":false},{"id":"d","text":"Changes require a system reboot to take effect","is_correct":false}]'::jsonb,
'Junos uses a two-phase configuration model: first you stage changes in candidate configuration, then you apply them atomically with the "commit" command.',
ARRAY['https://www.juniper.net/documentation/us/en/software/junos/cli/'],
...
'[{"id":"a","text":"show route 192.168.100.0/24","is_correct":true}]'::jsonb,
'The command "show route 192.168.100.0/24" displays all routes matching the prefix.',
...
'[{"id":"a","text":"ge (Gigabit Ethernet)","is_correct":true},{"id":"b","text":"xe (10 Gigabit Ethernet)","is_correct":true},{"id":"c","text":"gi (GigabitEthernet Cisco-style)","is_correct":false},{"id":"d","text":"fa (FastEthernet Cisco-style)","is_correct":false}]'::jsonb,
'Junos uses its own interface naming convention: ge- (Gigabit Ethernet), xe- (10 Gigabit Ethernet), et- (40/100 Gigabit Ethernet).',
...
'[{"id":"a","text":"then accept;","is_correct":false},{"id":"b","text":"then count;","is_correct":false},{"id":"c","text":"then count accept;","is_correct":true},{"id":"d","text":"then log;","is_correct":false}]'::jsonb,
'To count packets without dropping, you need both count AND accept actions: "then count accept;".',
...
'[{"id":"a","text":"To replace IP routing with label switching for faster forwarding","is_correct":true},{"id":"b","text":"To provide encryption for VPN traffic","is_correct":false},{"id":"c","text":"To replace BGP as the internet routing protocol","is_correct":false},{"id":"d","text":"To provide network address translation","is_correct":false}]'::jsonb,
'MPLS (Multiprotocol Label Switching) inserts a label between Layer 2 and Layer 3 headers. Routers forward packets based on labels rather than IP addresses.',
...
'[{"id":"a","text":"P router (Provider router)","is_correct":false},{"id":"b","text":"PE router (Provider Edge router)","is_correct":true},{"id":"c","text":"CE router (Customer Edge router)","is_correct":false},{"id":"d","text":"RR (Route Reflector)","is_correct":false}]'::jsonb,
'The PE (Provider Edge) router sits at the edge of the service provider network. It performs label imposition (push) on incoming packets.',
ARRAY['https://www.juniper.net/documentation/us/en/software/junos/mpls/'],
...
'[{"id":"a","text":"OSPF","is_correct":false},{"id":"b","text":"LDP (Label Distribution Protocol)","is_correct":true},{"id":"c","text":"RIP","is_correct":false},{"id":"d","text":"SNMP","is_correct":false}]'::jsonb,
'LDP (Label Distribution Protocol) is used to distribute MPLS label bindings between routers. RSVP-TE can also distribute labels while providing traffic engineering.',
ARRAY['https://www.juniper.net/documentation/us/en/software/junos/mpls/'],
...
'[{"id":"a","text":"90","is_correct":false},{"id":"b","text":"100","is_correct":false},{"id":"c","text":"110","is_correct":true},{"id":"d","text":"120","is_correct":false}]'::jsonb,
'OSPF has a default administrative distance of 110 in Cisco IOS.',
...
'[{"id":"a","text":"1-1001","is_correct":true},{"id":"b","text":"1-4094","is_correct":false},{"id":"c","text":"1002-4096","is_correct":false},{"id":"d","text":"1-1005","is_correct":false}]'::jsonb,
'Normal-range VLANs on Cisco switches are VLAN 1-1001. VLANs 1002-1005 are reserved for legacy Token Ring and FDDI.',
...
'[{"id":"a","text":"router rip","is_correct":true},{"id":"b","text":"enable rip","is_correct":false},{"id":"c","text":"rip enable","is_correct":false},{"id":"d","text":"ip routing rip","is_correct":false}]'::jsonb,
'The "router rip" command enters RIP configuration mode in Cisco IOS.',
