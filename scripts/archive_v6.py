...
EXAM = {
    ...
    "JN0-230":   ("b0000000-0000-0000-0000-000000000022", "a0000000-0000-0000-0000-000000000003", "JNCIA-SEC"),
    "JN0-480":   ("b0000000-0000-0000-0000-000000000020", "a0000000-0000-0000-0000-000000000004", "JNCIA-DC"),
    "JN0-223":   ("b0000000-0000-0000-0000-000000000021", "a0000000-0000-0000-0000-000000000005", "JNCIA-AUT"),
...
def _multi_gen(eid, tid, gen_funcs, seed, targets=None):
    ...
    for i, (func, target) in enumerate(gen_funcs):
        ...
        qs = func(gs)
        # Dedup within this template batch only
        seen_in_batch = set()
        batch = []
        for q in qs:
            b = q["body"]
            if b not in seen_in_batch:
                seen_in_batch.add(b)
                batch.append(q)
        batch = batch[:target]
        all_q.extend(batch)
